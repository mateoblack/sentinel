...
// DefaultSessionTableName is the default name for the server sessions table.
const DefaultSessionTableName = "sentinel-sessions"

// InitSessionsCommandInput contains the input for the init sessions command.
type InitSessionsCommandInput struct {
	TableName   string
	Region      string
	AWSProfile  string
	Plan        bool
...
// ConfigureInitSessionsCommand sets up the init sessions command as a subcommand of init.
func ConfigureInitSessionsCommand(app *kingpin.Application, s *Sentinel) {
	...
	// Create sessions as a subcommand of init
	cmd := initCmd.Command("sessions", "Provision the DynamoDB server sessions table")
	...
	cmd.Action(func(c *kingpin.ParseContext) error {
		err := InitSessionsCommand(context.Background(), input)
		app.FatalIfError(err, "init sessions")
		return nil
	})
}

// InitSessionsCommand executes the init sessions command logic.
// It creates the DynamoDB server sessions table with correct schema, GSIs, and TTL configuration.
func InitSessionsCommand(ctx context.Context, input InitSessionsCommandInput) error {
	// Set up I/O
	stdout := input.Stdout
	if stdout == nil {
		stdout = os.Stdout
	...
	if input.GenerateIAM {
		policy := generateSessionTableIAMPolicy(input.TableName, input.Region)
	...
	// Get schema
	schema := infrastructure.SessionTableSchema(input.TableName)
	...
	if input.Plan {
		...
		fmt.Fprintln(stdout, "")
		fmt.Fprintln(stdout, "Sentinel Init Sessions - Plan")
	...
	fmt.Fprintln(stdout, "")
	fmt.Fprintln(stdout, "Sentinel Init Sessions")
	fmt.Fprintln(stdout, strings.Repeat("=", 50))
	fmt.Fprintln(stdout, "")
	fmt.Fprintf(stdout, "Table:        %s\n", schema.TableName)
...
// generateSessionTableIAMPolicy generates an IAM policy document for DynamoDB session table access.
// The policy includes permissions for table creation, status checks, and data operations.
func generateSessionTableIAMPolicy(tableName, region string) string {
	...
	policy := IAMPolicyDocument{
		...
		Statement: []IAMPolicyStatement{
			{
				Sid:    "SentinelSessionTableProvisioning",
				Effect: "Allow",
				Action: []string{
			...
			{
				Sid:    "SentinelSessionTableOperations",
				Effect: "Allow",
				Action: []string{
					"dynamodb:GetItem",
					"dynamodb:PutItem",
