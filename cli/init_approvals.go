...
// DefaultApprovalTableName is the default name for the approval requests table.
const DefaultApprovalTableName = "sentinel-requests"

// InitApprovalsCommandInput contains the input for the init approvals command.
type InitApprovalsCommandInput struct {
	TableName   string
	Region      string
	AWSProfile  string
	Plan        bool
...
// IAMPolicyDocument represents an IAM policy document structure.
...
// ConfigureInitApprovalsCommand sets up the init approvals command as a subcommand of init.
func ConfigureInitApprovalsCommand(app *kingpin.Application, s *Sentinel) {
	...
	// Create approvals as a subcommand of init
	cmd := initCmd.Command("approvals", "Provision the DynamoDB approval requests table")
	...
	cmd.Action(func(c *kingpin.ParseContext) error {
		err := InitApprovalsCommand(context.Background(), input)
		app.FatalIfError(err, "init approvals")
		return nil
	})
}

// InitApprovalsCommand executes the init approvals command logic.
// It creates the DynamoDB approval requests table with correct schema, GSIs, and TTL configuration.
func InitApprovalsCommand(ctx context.Context, input InitApprovalsCommandInput) error {
	// Set up I/O
	stdout := input.Stdout
	if stdout == nil {
		stdout = os.Stdout
	...
	if input.GenerateIAM {
		policy := generateTableIAMPolicy(input.TableName, input.Region)
	...
	// Get schema
	schema := infrastructure.ApprovalTableSchema(input.TableName)
	...
	if input.Plan {
		...
		fmt.Fprintln(stdout, "")
		fmt.Fprintln(stdout, "Sentinel Init Approvals - Plan")
	...
	fmt.Fprintln(stdout, "")
	fmt.Fprintln(stdout, "Sentinel Init Approvals")
	fmt.Fprintln(stdout, strings.Repeat("=", 50))
	fmt.Fprintln(stdout, "")
	fmt.Fprintf(stdout, "Table:        %s\n", schema.TableName)
	...
	fmt.Fprintf(stdout, "  1. Configure --request-table %s in your Sentinel commands\n", input.TableName)
	fmt.Fprintln(stdout, "  2. Or set SENTINEL_REQUEST_TABLE environment variable")
	fmt.Fprintln(stdout, "")
	fmt.Fprintf(stdout, "  Run 'sentinel init approvals --generate-iam --table %s --region %s'\n", input.TableName, input.Region)
	fmt.Fprintln(stdout, "  to generate the IAM policy document for table access.")

	return nil
}

// generateTableIAMPolicy generates an IAM policy document for DynamoDB table access.
// The policy includes permissions for table creation, status checks, and data operations.
func generateTableIAMPolicy(tableName, region string) string {
	...
	policy := IAMPolicyDocument{
		...
		Statement: []IAMPolicyStatement{
			{
				Sid:    "SentinelApprovalTableProvisioning",
				Effect: "Allow",
				Action: []string{
			...
			{
				Sid:    "SentinelApprovalTableOperations",
				Effect: "Allow",
				Action: []string{
					"dynamodb:GetItem",
					"dynamodb:PutItem",
